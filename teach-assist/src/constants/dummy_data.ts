export const summary =  `Why Memory Management?
...
export const feedback = `Your answer is technically correct and would score well in a 1 mark question, however MMU translates logical addresses into physical address," is lacking in several key areas compared to the more detailed response. Firstly, it is incomplete as it doesn't mention the runtime aspect of the translation process or the bounds checking performed by the MMU. To improve, it should state that the translation occurs at runtime and that the MMU ensures addresses are within set limits. Secondly, it lacks technical accuracy since it omits the crucial role of base and limit registers in memory protection. Including this detail would enhance the technical precision. Thirdly, the answer is vague and could confuse readers unfamiliar with the terms. Adding details on how logical addresses are converted to physical addresses would improve clarity. Fourthly, it lacks contextual information about the importance of address translation in the system's overall functioning. Providing context on why address translation is necessary and its role in memory management would be beneficial.`